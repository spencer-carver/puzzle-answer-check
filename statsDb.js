...
async function addIntermediate(puzzleName) {
    ...
    if (!record) {
        ...
        record = { stats: { M: { ...record.stats.M, intermediates: { N: `${ parseInt(record.stats.M.intermediates.N) + 1 }` } } } };
    }

    return db.insertItem(generateParams(puzzleName, record.stats));
}

async function addIncorrect(puzzleName) {
    let record = await db.queryItem(generateParams(puzzleName));

    if (!record) {
        record = { stats: { M: { incorrect: { N: "1" } } } };
    } else if (!record.stats.M.incorrect) {
        record = { stats: { M: { ...record.stats.M, incorrect: { N: "1" } } } };
    } else {
        record = { stats: { M: { ...record.stats.M, incorrect: { N: `${ parseInt(record.stats.M.incorrect.N) + 1 }` } } } };
    }
...
async function addCorrect(puzzleName) {
    ...
    if (!record) {
        record = { stats: { M: { correct: { N: "1" }, firstSolveTimestamp: { N: (new Date()).getTime() } } } };
    } else if (!record.stats.M.correct) {
        record = { stats: { M: { ...record.stats.M, correct: { N: "1" }, firstSolveTimestamp: { N: `${ (new Date()).getTime() }` } } } };
    } else {
        record = { stats: { M: { ...record.stats.M, correct: { N: `${ parseInt(record.stats.M.correct.N) + 1 }` } } } };
    }
